...
def parse_args():
    ...
    parser.add_argument('--model_path', action='store',
                        default='./Model_trained/W2VMLP_lr_0.000010_b=8_LSTM_{246}.bak', dest='model_path',
...
def main():
    opt = parse_args()
    log_f = open(
        './Logs/W2VMLP/SGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.bak' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)),
        'w+')
    ...
    for count, doc in enumerate(X):
        ...
        if suc != 0 and ((count + 1) - NoAttack) != 0:
            ...
            pickle.dump(changed_words_all,
                        open( "./Output/W2VMLP/"+ 'SGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.pickle' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)), 'wb'))
