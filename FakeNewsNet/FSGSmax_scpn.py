...
class Attacker(object):
    ...
    def attack(self, data, y, length, sentence_neighbors, sentences):
        ...
        while success_flag == 1:
            ...
            for visit_idx in range(min(len(sentences), 50)):
...
output_file = './Logs/%s/%s/%s/%s/' % (Dataset, Model_Type, model_name, 'FSGS')
...
write_file(Dataset, Model_Type, model_name, budget, 'FSGS', time_limit)
