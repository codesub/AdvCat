...
def parse_args():
    ...
    parser.add_argument('--model_path', action='store', default='./Model_trained/BertLSTM_lr_0.000100_b=1_LSTM_{58}.bak',
                        dest='model_path',
...
class Attacker(object):
        ...
        for i,candi_sample_index in enumerate(poses):
            ...
            if len(candidates) != 1:
                ...
                for cand in candidates:
                    sample = [[i] for i in cand if i.strip()]
                    sample = client.encode(sample, is_tokenized=True)
                    sample_text.append(sample)
                sample_text = torch.as_tensor(sample_text)
                preds = self.model(sample_text.cuda().transpose(1,0))
    ...
    def attack(self, count, doc, y, NoAttack, log_f, N_REPLACE):
        ...
        sample_text = [[i] for i in words if i.strip() ]
        sample_text = torch.from_numpy(client.encode(sample_text,is_tokenized=True))
        orig_prob, orig_pred = classifybert(sample_text, self.model)
        orig_pred = orig_pred.data.cpu().numpy()
        pred, pred_prob = orig_pred, orig_prob

        # y = int(0) if y == ["FALSE"] else int(1)
        ...
        for n in range(RN):
            ...
            K_set = random.sample(allCode, N_RS)
            ...
            sample_text_new = [[i] for i in new_words if i.strip()]
            sample_text_new = torch.from_numpy(client.encode(sample_text_new, is_tokenized=True))
            orig_prob_new, orig_pred_new = classifybert(sample_text_new, self.model)
        ...
        return success, RN, num_armchain, time_success, arm_chains, arm_preds,np.array(n_change)/len(allCode_ori),NoAttack,arm_chain_words,arm_preds_class,N_RS


def main():
    opt = parse_args()
    log_f = open(
        './Logs/BertLSTM/All_UCB_R_L_Budget_%s_N=%s_TAU=%s_N_REPLACE=%s_Time=%s_positive.bak' % (
    ...
    for count, doc in enumerate(X):
        ...
        success, RN, num_armchain, time_success, arm_chains, arm_preds,n_change,NoAttack_num,arm_chain_words,arm_preds_class,N_RS= attacker.attack(count, doc, y[count],
        ...
        if len(success_pos) >= 1:
            ...
            Total_Query = Total_Query + (N_RS * N_NEIGHBOR + AI_sample)
        ...
        if (count - NoAttack_num) != 0 and success_num != 0:
            ...
            pickle.dump(changed_words_all,
                        open("./Output/BertLSTM/" + 'All_UCB_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.pickle' % (
