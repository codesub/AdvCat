...
def parse_args():
    ...
    parser.add_argument('--model_path', action='store',
                        default='./Model_trained/BertLSTM_lr_0.000100_b=1_LSTM_{58}.bak', dest='model_path',
...
class Attacker(object):
    ...
    def attack(self, count, doc, y, NoAttack,log_f):
        ...
        if int(len(Set_residual)-N_REPLACE * len(Set_residual)) >= 1:
            Set_residual = np.random.choice(Set_residual,int(len(Set_residual)-N_REPLACE * len(Set_residual)))
...
def main():
    opt = parse_args()
    log_f = open(
        './Logs/BertLSTM/All_SGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.bak' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)),
        'w+')

    TITLE = '=== ' + 'SGS' + ' target prob = ' + str(TAU) + ' changeRange = ' + str(N_REPLACE) + ' ==='
    ...
    for count, doc in enumerate(X):
        ...
        if suc != 0 and ((count + 1) - NoAttack) != 0:
            ...
            pickle.dump(changed_words_all,
                        open( "./Output/BertLSTM/"+ 'All_SGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.pickle' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)), 'wb'))
