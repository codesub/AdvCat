...
def main():
    opt = parse_args()
    log_f = open(
        './Logs/W2VCNN/RSUCB_R_L_Budget_%s_N=%s_TAU=%s_N_REPLACE=%s_Time=%s_positive_updatesuccess_count.bak' % (
    ...
    for count, doc in enumerate(X):
        ...
        if (count - NoAttack_num) != 0 and success_num != 0:
            ...
            pickle.dump(changed_words_all,
                        open("./Output/W2VCNN/" + 'RSUCB_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.pickle' % (
