...
def main():
    opt = parse_args()
    log_f = open(
        './Logs/W2VMLP/FSGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.bak' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)),
        'w+')
    ...
    for count, doc in enumerate(X):
        ...
        if suc != 0 and ((count + 1) - NoAttack) != 0:
            ...
            pickle.dump(changed_words_all,
                        open( "./Output/W2VMLP/"+ 'FSGS_Budget=%s_N=%s_TAU=%s_N_REPLACE=%d_SECONDS=%s.pickle' % (str(Budget),str(N_NEIGHBOR), str(TAU), N_REPLACE,str(SECONDS)), 'wb'))
